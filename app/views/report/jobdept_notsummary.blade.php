...
<div class="row">
	<div id="breadcrumb" class="col-xs-12">
		<ol class="breadcrumb">
			...
			<li><a href="#">รายงาน JOB ยังไม่สรุปผล จำแนกหน่วยงาน</a></li>
		</ol>
	</div>
</div>
<div class="row">
		...
		<div class="box">
			<div class="box-header">
				<div class="box-name">
					...
					<span>รายงาน JOB ยังไม่สรุปผล จำแนกหน่วยงาน</span>
				</div>
			...
			<div class="box-content">
				<div class="row">
					...
					<div class="col-xs-12">
						<form method="POST" action="{{URL::to('report/repair/job-dept-notsummary-export')}}">
							<div class="row">
								<div class="col-xs-12">
							...
							<div class="row">
								<div class="col-xs-12">
									<label class="col-xs-2 label-control">เลือกแผนก</label>
									<div class="col-xs-2">
										<select id="subdept" name="subdept">
											<option value="all">--ทั้งหมด--</option>
											@foreach($subdept as $s)
											<option value="{{$s->sub_dept_id}}">{{$s->sub_dept_name}}</option>
											@endforeach
										</select>
									</div>
...
<script type="text/javascript">
	...
	function TableStatus(){
		var table = $('#tbl').dataTable( {
	        ...
	        "serverSide": true,
	        "iDisplayLength": 20,
	        "ajax": "{{URL::to('report/repair/job-dept-notsummary-data')}}",
	        "columnDefs": [
			{
            	render: function ( data,type,obj,row) {

                    return "<a class='ajax-unlink' target='_blank' href='{{URL::to('durable/repair-report-add')}}/"+obj.technic_name+"' ><i class='fa fa-wrench  text-info'></i></a>"
                },targets: 0, orderable: false
            }
            ],
	        "columns": [
	        	{ "data": "repair_id" ,"width":"1%","class":"text-center"},
	        	{ "data": "durable_name" ,"width":"14%","class":"text-left"},
	        	{ "data": "ruin" ,"width":"15%","class":"text-left"},
	        	{ "data": "durable_id" ,"width":"15%","class":"text-left"},
	        	{ "data": "repair_cost" ,"width":"10%","class":"text-left"},
	...
	$(document).ready(function() {
		...
		$('#datepicker,#datepicker2').datepicker({
	    	...
	    	reloadtable($("#subdept").val());
		});

	    $("#subdept").change(function(e){
	    ...
	    function reloadtable(tech)
	    {
	    	...
	    	if(date1!="Invalid Date" && date2 != "Invalid Date")
	    	{
		    	...
		    	$('#tbl').dataTable().api().ajax.url("{{URL::to('report/repair/job-dept-notsummary-data/')}}/"+tech+"/"+ds1+"/"+ds2).load();
	    	}
	    }
