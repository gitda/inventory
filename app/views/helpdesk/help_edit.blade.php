							...
							<div class="col-sm-4">
								<select class="form-control select2" id="ruin_type_id" name="ruin_type_id" required>
						      		...
						      		@foreach($ruin_type as $rt)
						      	 	<option value="{{$rt->ruin_type_id}}" @if($rt->ruin_type_id==$helpdesk->helpdesk_type_id) {{'selected'}} @endif>{{$rt->ruin_type_name}}</option>
						      		@endforeach
						      	</select>
							</div>	
							<div class="col-sm-4">
<!-- 								<select id="symptoms_id" name="symptoms_id" class="form-control" required>
 ...
 -->
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<textarea  class="form-control" id="help_description" name="help_description" rows="5" required>{{$helpdesk->help_description}}</textarea>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-8">
								<select class="form-control select2" id="sub_dept_id" name="sub_dept_id" required>
						      		...
						      		@foreach($sub_dept as $sd)
						      	 	<option value="{{$sd->sub_dept_id}}" @if($helpdesk->sub_dept_id==$sd->sub_dept_id) {{'selected'}} @endif>{{$sd->sub_dept_name}}</option>
						      		@endforeach
						      	</select>
							</div>	
						</div>
							...
							<div class="col-sm-4">
								<select class="form-control" id="reslove_type" name="reslove_type" required>
						      		...
						      		@foreach($reslove_type as $st)
						      	 	<option value="{{$st->reslove_type}}" @if($helpdesk->reslove_type==$st->reslove_type) {{'selected'}} @endif>{{$st->reslove_name}}</option>
						      		@endforeach
						      	</select>
							</div>
							<div class="col-sm-4">
								<select class="form-control" id="cause" name="cause" required>
						      		...
						      		@foreach($cause as $c)
						      	 	<option value="{{$c->cause_id}}" @if($helpdesk->cause==$c->cause_id) {{'selected'}} @endif>{{$c->cause_name}}</option>
						      		@endforeach
						      	</select>
							</div>
							<div class="col-sm-4">
							...
							<div class="col-sm-4">
								<select class="form-control" id="staff_id" name="staff_id" required>
						      		...
						      		@foreach($technician as $tn)
						      	 	<option value="{{$tn->technic_id}}" @if($helpdesk->staff_id==$tn->technic_id) {{'selected'}} @endif>{{$tn->technic_name}}</option>
						      		@endforeach
						      	</select>
							</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<textarea  class="form-control" id="help_note" name="help_note" rows="3">{{$helpdesk->help_note}}</textarea>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select class="form-control" id="help_result" name="help_result" required>
						      		...
						      		@foreach($help_result as $hr)
						      	 	<option value="{{$hr->help_result_id}}" @if($helpdesk->help_result==$hr->help_result_id) {{'selected'}} @endif>{{$hr->help_result_name}}</option>
						      		@endforeach
						      	</select>
							</div>

						</div>
						@if($helpdesk->repair_id <> "")
						<div class="form-group ">
							<label for="help_result" class="col-sm-2 control-label">ส่งซ่อม</label>
							<div class="col-sm-4">
								<a class="" target="_blank" href="{{URL::to('durable/repair-edit/'.$helpdesk->repair_id)}}">บันทึกซ่อมแล้ว</a>
							</div>
						</div>
						@endif
...
$(document).ready(function() {
	...
	fnResloveType({{$helpdesk->reslove_type}});
});
</script>
@stop
